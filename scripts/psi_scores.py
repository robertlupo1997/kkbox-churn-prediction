...
def psi(a, e, bins=10):
    ...
    c1 = np.histogram(a, bins=edges)[0] / len(a)
    c2 = np.histogram(e, bins=edges)[0] / len(e)
    c1 = np.clip(c1, 1e-6, None)
    c2 = np.clip(c2, 1e-6, None)
...
def main():
    ...
    for m in models:
        ...
        refp = min(
            model_scores, key=lambda p: re.findall(r"scores_(\d{4}-\d{2}-\d{4}-\d{2})", p) or ["zz"]
        )
            ...
            rows.append(
                {
                    ...
                    "psi": psi_val,
                    "reference": refp.split("scores_")[1]
                    .split(f"_{m}.csv")[0]
                    .replace("-", "→", 1),
                }
            )
    ...
    if rows:
        ...
        if len(high_drift) > 0:
            print("⚠️  High PSI drift (>0.2) detected:")
