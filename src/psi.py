...
def main() -> None:
    ...
    ap.add_argument(
        "--features",
        required=True,
        help="CSV of features for each window concatenated with a 'window' column, or a glob like eval/features_*.csv",
    )
    ap.add_argument(
        "--ref", default=None, help="Reference window name (defaults to first alphabetically)"
    )
    ...
    # Load and concat
    paths = (
        sorted([p for p in Path(".").glob(args.features)])
        if "*" in args.features
