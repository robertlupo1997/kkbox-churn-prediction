...
class ModelTrainer:
    ...
    def save_models(self, output_dir: str) -> None:
        ...
        for name, model in self.models.items():
            ...
            if name == "xgboost":
                try:
                    ...
                    print("   Saved XGBoost in JSON format for compatibility")
                except Exception as e:
                    print(f"    Could not save XGBoost JSON format: {e}")
        ...
        try:
            ...
            print("   Saved feature_encoders.pkl")
        except Exception as e:
...
def run_training_pipeline(
    ...
    if use_temporal_split and HAS_TEMPORAL_CV and "cutoff_ts" in df.columns:
        ...
        if len(train_idx) == 0 or len(val_idx) == 0:
            print(" Temporal split failed (insufficient data), falling back to random split")
    ...
    if HAS_TEMPORAL_CV:
        try:
            ...
            print(
                f"  Log Loss: {bootstrap_ci['log_loss']['mean']:.4f} "
                f"({bootstrap_ci['log_loss']['ci_lower']:.4f} - {bootstrap_ci['log_loss']['ci_upper']:.4f})"
            )
            print(
                f"  AUC: {bootstrap_ci['auc']['mean']:.4f} "
