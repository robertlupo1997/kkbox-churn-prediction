...
class ModelCalibrator:
    ...
    def calibrate_all_models(self, features_path: str, models_dir: str) -> dict[str, Any]:
        ...
        # Handle categorical encoding (skip if already numeric)
        categorical_features = ["gender", "payment_method_latest", "registered_via", "city"]
        for col in categorical_features:
            if col in X.columns:
                # Skip if already numeric (comprehensive features are pre-encoded)
                if X[col].dtype in ["int32", "int64", "float64"]:
                    continue
                # Only encode if we have an encoder and column is not numeric
    ...
    def save_calibrated_models(self, calibration_results: dict[str, Any], output_dir: str) -> None:
        ...
        for model_name, results in calibration_results.items():
            ...
            try:
                ...
                print(f"    Failed to save calibrator for {model_name}: {e}")
...
def run_calibration_pipeline(
    ...
    print("=" * 50)
    print(f"{'Model':<20} {'Log Loss D':<12} {'Brier D':<10} {'ECE D':<10}")
    print("-" * 50)
