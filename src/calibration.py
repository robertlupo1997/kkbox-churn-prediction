...
class ModelCalibrator:
    ...
    def calibrate_model(self, model, X_cal: pd.DataFrame, y_cal: pd.Series,
        ...
        results = {
            ...
            'improvement': {
                ...
                'brier_delta': calibrated_metrics['brier_score'] - uncalibrated_metrics['brier_score'],
    ...
    def calibrate_all_models(self, features_path: str, models_dir: str) -> Dict[str, Any]:
        ...
        for model_name, model in models.items():
            ...
            try:
                results = self.calibrate_model(
                    model, X_cal, y_cal, X_test, y_test, model_name
                )
    ...
    def save_calibrated_models(self, calibration_results: Dict[str, Any], 
                              output_dir: str) -> None:
...
def run_calibration_pipeline(features_path: str = "features/features_processed.csv",
    ...
    for model_name, results in calibration_results.items():
        improvement = results['improvement']
        print(f"{model_name:<20} {improvement['log_loss_delta']:+8.4f}    "
    ...
    if calibration_results:
        best_improvement = min(
            ...
            key=lambda x: x[1]['improvement']['brier_delta']
        )
    ...
    summary = {
        ...
        'total_improvements': len([
            r for r in calibration_results.values() 
            if r['improvement']['brier_delta'] < 0
        ])
    }
