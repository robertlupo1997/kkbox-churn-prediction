...
class ChurnErrorAnalyzer:
    ...
    def analyze(self, y_true: np.ndarray, y_pred: np.ndarray,
        ...
        # Build results
        results = {
            'summary': self._compute_summary(y_true, y_pred, y_pred_binary,
                                            correct, tp, tn, fp, fn),
            'confidence_analysis': self._analyze_confidence(y_true, y_pred, correct),
            'segment_analysis': self._analyze_segments(y_true, y_pred, correct,
                                                      features_df, feature_names),
            'hardest_examples': self._find_hardest_examples(y_true, y_pred,
                                                           features_df),
            'business_impact': self._compute_business_impact(tp, tn, fp, fn),
            'recommendations': self._generate_recommendations(y_true, y_pred,
                                                            correct, features_df,
                                                            feature_names)
        }
    ...
    def _compute_summary(self, y_true, y_pred, y_pred_binary,
                        correct, tp, tn, fp, fn) -> Dict[str, Any]:
    ...
    def _analyze_segments(self, y_true, y_pred, correct,
                         features_df, feature_names) -> Dict[str, List[Dict]]:
    ...
    def _find_hardest_examples(self, y_true, y_pred, features_df,
                              n_examples: int = 20) -> Dict[str, pd.DataFrame]:
    ...
    def _compute_business_impact(self, tp, tn, fp, fn) -> Dict[str, float]:
        ...
        return {
            ...
            'cost_vs_all_positive': float(total_cost / all_positive_cost) if all_positive_cost > 0 else 0,
            'cost_vs_all_negative': float(total_cost / all_negative_cost) if all_negative_cost > 0 else 0,
            'savings_vs_baseline': float(min(all_positive_cost, all_negative_cost) - total_cost)
        }

    def _generate_recommendations(self, y_true, y_pred, correct,
                                 features_df, feature_names) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("No major issues detected. Consider hyperparameter tuning for incremental gains.")
    ...
    def print_report(self, results: Dict[str, Any], verbose: bool = True):
        ...
        if verbose:
            ...
            for bin_info in conf['bins']:
                print(f"  {bin_info['bin']}: {bin_info['n_samples']:>5} samples, "
        ...
        for feature, segments in segment_analysis.items():
            ...
            if segments and segments[0]['accuracy'] < summary['accuracy'] - 0.05:
                worst = segments[0]
                print(f"  {feature}='{worst['segment']}': {worst['accuracy']:.2%} accuracy "
...
def run_error_analysis(y_true: np.ndarray, y_pred: np.ndarray,
                      features_df: pd.DataFrame,
                      threshold: float = 0.5,
                      fp_cost: float = 10.0,
                      fn_cost: float = 50.0) -> Dict[str, Any]:
    """
