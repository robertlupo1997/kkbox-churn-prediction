...
def mismatch_audit(labels_df: pd.DataFrame, max_examples: int = 50) -> pd.DataFrame:
    ...
    audit_df = mismatches[['msno', 'last_expire_date', 'next_txn_date', 'days_to_next', 
                          'is_churn', 'official_is_churn']].copy()
    audit_df.rename(columns={
        'is_churn': 'generated_label',
        'official_is_churn': 'official_label'
    }, inplace=True)
    
    # Sort by severity: false negatives first (we missed churners), then false positives
    audit_df['mismatch_type'] = audit_df.apply(lambda row: 
        'false_negative' if row['generated_label'] < row['official_label'] else 'false_positive', axis=1)
    
    audit_df = audit_df.sort_values(['mismatch_type', 'days_to_next'], ascending=[True, True])
    
    print(f"\n🔍 MISMATCH AUDIT REPORT:")
    print(f"Total mismatches: {len(mismatches):,}")
    print(f"False negatives (Generated=0, Official=1): {len(audit_df[audit_df['mismatch_type']=='false_negative']):,}")
    print(f"False positives (Generated=1, Official=0): {len(audit_df[audit_df['mismatch_type']=='false_positive']):,}")
    ...
    for _, row in top_examples.iterrows():
        ...
        next_txn_str = row['next_txn_date'].strftime('%Y-%m-%d') if pd.notna(row['next_txn_date']) else "None"
        print(f"  {row['msno']}: expire={row['last_expire_date']}, next_txn={next_txn_str}, "
...
def analyze_mismatches(labels_df: pd.DataFrame, max_examples: int = 10):
    ...
    print(f"Generated=1, Official=0: {len(mismatches[mismatches['is_churn'] > mismatches['official_is_churn']]):,}")
    print(f"Generated=0, Official=1: {len(mismatches[mismatches['is_churn'] < mismatches['official_is_churn']]):,}")
    
    if max_examples > 0:
        print(f"\nFirst {min(max_examples, len(mismatches))} mismatches:")
        print(mismatches[['msno', 'is_churn', 'official_is_churn', 'last_expire_date', 'next_txn_date']].head(max_examples))
...
def main():
    ...
    parser.add_argument(
        "--cutoff-date",
        default="2017-03-01",
        help="Cutoff date for label generation (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--window-days",
        type=int,
        default=30,
        help="Days after expiration to check for renewals"
    )
    parser.add_argument(
        "--output",
        default="data/generated_labels.csv",
