...
def load_model():
    ...
    try:
        import joblib
        for name, p in [("random_forest","models/random_forest.pkl"), ("logreg","models/logistic_regression.pkl")]:
            fp = Path(p)
            if fp.exists():
...
def get_action_recommendation(score: float, rules: dict = None):
    ...
    if not rules:
        if score > 0.7:
            return "🚨 High-priority retention campaign", "Contact customer success team immediately"
        elif score > 0.4:
...
if clf is None:
    st.error("❌ **No trained model found.** Please ensure models are saved in `models/` directory.")
    st.info("Expected files: `models/xgboost.pkl`, `models/random_forest.pkl`, or `models/logistic_regression.pkl`")
    st.stop()

if feat_df.empty:
    st.error("❌ **No feature data found.** Please save processed features to `eval/app_features.csv`.")
    st.info("Generate features using: `python3 src/features_processor.py` and copy output to `eval/app_features.csv`")
    st.stop()
        ...
        if row.empty:
            ...
            try:
                ...
                with st.expander("🔍 Feature Details"):
                    ...
                    st.dataframe(
                        feature_vals.to_frame("Value").reset_index().rename(columns={"index": "Feature"}),
        ...
        try:
            ...
            if "msno" not in df_upload.columns:
                ...
                if df_scored.empty:
                    ...
                    # Show results table
                    st.dataframe(
                        results[["msno", "churn_probability", "risk_level", "recommended_action"]].head(50),
                        use_container_width=True
                    )
                    ...
                    if not missing_features.empty:
                        st.warning(f"⚠️ {len(missing_features)} members skipped (not in feature database)")
...
st.caption("🔒 **Privacy**: No raw KKBOX data exposed. Features processed with temporal safeguards.")